namespace CodeBase.Infrastructure.Services.Assets
{
  public static class AssetPath
  {
    public const string Hud = "Prefabs/HUD";
    
    public const string MonthContainer = "Prefabs/ParticularMonthContainer";
    
    public const string HolidayDataAssembly = "Prefabs/Holiday/HolidayDataAssembly";
    
    public const string HeaderNoName = "Prefabs/Holiday/HolidayComponents/HeaderNoName";
    public const string HeaderWithName = "Prefabs/Holiday/HolidayComponents/HeaderWithName";

    public const string IconImage = "Prefabs/Holiday/HolidayComponents/IconImage";
    
    public const string HolidayName = "Prefabs/Holiday/HolidayComponents/HolidayName";

    public const string Suggestion = "Prefabs/Holiday/HolidayComponents/Suggestion";
    public const string SuggestionItem = "Prefabs/Holiday/HolidayComponents/Elements/Suggestion_image";
    
    public const string GeneralContentText = "Prefabs/Holiday/HolidayComponents/GeneralContentText";
    
  }
}